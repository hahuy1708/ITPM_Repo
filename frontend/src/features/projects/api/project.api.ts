...
import type { ApiResponse, PaginatedResponse, Project, ProjectStatus, ProjectVisibility, TaskGroup } from '@/types';
...
export const projectService = {
  ...
  getTaskGroups: async (id: string, token: string): Promise<ApiResponse<TaskGroup[]>> => {
    const response = await fetch(getApiUrl(API_ENDPOINTS.PROJECTS.TASK_GROUPS(id)), {
      headers: authHeaders(token),
    });

    return readJson<ApiResponse<TaskGroup[]>>(response);
  },

  createTaskGroup: async (id: string, data: { name: string; key?: string }, token: string): Promise<ApiResponse<TaskGroup[]>> => {
    const response = await fetch(getApiUrl(API_ENDPOINTS.PROJECTS.TASK_GROUPS(id)), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  reorderTaskGroups: async (id: string, groupKeys: string[], token: string): Promise<ApiResponse<TaskGroup[]>> => {
    const response = await fetch(getApiUrl(`${API_ENDPOINTS.PROJECTS.TASK_GROUPS(id)}/reorder`), {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        ...authHeaders(token),
